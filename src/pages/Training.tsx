...
const TrainingPage = () => {
  ...
  const loadWorkouts = useCallback(async () => {
    ...
    if (error || !workoutRows) { toast.error('Não foi possível carregar seus treinos.'); setLoading(false); return; }
  ...
  const startWorkout = async () => {
    ...
    if (error || !workoutRow) { toast.error('Não foi possível criar o treino.'); setSaving(false); return; }
    ...
    for (const [position,exercise] of draftExercises.entries()) {
      ...
      if (exerciseError || !exerciseRow) { toast.error('Não foi possível salvar todos os exercícios.'); setSaving(false); return; }
      ...
      if (setError || !setRows) { toast.error('Não foi possível salvar as séries.'); setSaving(false); return; }
  ...
  const updateActiveSet = async (exerciseId:string,setId:string,patch:Partial<TrainingSet>) => {
    ...
    await supabase.from('training_sets').update(dbPatch).eq('id',setId);
  };

  const completeWorkout = async () => {
    if (!activeWorkout) return;
    const completed={...activeWorkout,status:'completed' as const,completedAt:new Date().toISOString()};
    if (user) await supabase.from('training_workouts').update({status:'completed',completed_at:completed.completedAt}).eq('id',completed.id);
